...
class TestJVLogger:
    ...
    def test_setup_logging_with_defaults(self, mocker):
        ...
        mock_rotating_handler.assert_called_once_with(
            '/tmp/jac_cloud_logs/jivas.log',
            maxBytes=5242880,
            backupCount=5
        )

    def test_invalid_log_level_defaults_to_info(self, mocker):
        # Mock root logger
        mock_root_logger = mocker.patch('logging.getLogger')
        mock_logger = mocker.MagicMock()
        mock_root_logger.return_value = mock_logger
    ...
    def test_colored_console_formatter_format(self, mocker):
        # Mock a log record
        ...
        assert formatted_message.startswith(f"{expected_color}INFO{reset_color}: Test message")
    
    def test_json_formatter_output(self, mocker):
        ...
        mocker.patch.object(JVLogger.JSONFormatter, 'formatTime', return_value="2023-10-01 12:00:00")
